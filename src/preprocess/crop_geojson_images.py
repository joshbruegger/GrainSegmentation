...
def _slice_array(
    ...
    if layout == "hwc":
        return np.asarray(
            arr[crop_top:crop_bottom, crop_left:crop_right, :]
        )
    if layout == "chw":
        return np.asarray(
            arr[:, crop_top:crop_bottom, crop_left:crop_right]
    ...
    with tifffile.TiffFile(input_path) as tif:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    "zarr is required for tiled TIFF reading"
                ) from exc
...
def main() -> None:
    ...
    print(f"Wrote GeoJSON: {args.out_geojson}")
